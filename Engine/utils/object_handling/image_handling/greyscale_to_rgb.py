...
def greyscaleToRGBA(greyscale_img: np.ndarray):
    image_size_x = greyscale_img.shape[1]
    image_size_y = greyscale_img.shape[0]
    rgb_image = np.zeros((image_size_y, image_size_x, 4))
    for x_position in prange(image_size_x):
        for y_position in range(image_size_y):
            greyscale_pixel_value = greyscale_img[y_position, x_position]
            # setting each colour channel to the value of teh greyscale image
            rgb_image[y_position, x_position] = [greyscale_pixel_value,
                                                 greyscale_pixel_value,
                                                 greyscale_pixel_value, 255]
    return rgb_image.astype(np.uint8) #converting to correct type first